...
In contrast, this chapter discusses ways in which this thesis adheres to and promotes best practices for reproducibility and auditability.
...
V\&V methods can broadly be classed into several categories: formal verification, simulation, empirical modelling, and static or dynamic analysis.
Fields like aviation, automotive, and medicine deploy some combination of these to mitigate and control risks on a daily basis.
...
One well-known example is the Boeing 777, which was the first commercial aircraft to be entirely designed using computer-aided design (CAD) and simulation tools, minimizing the need for physical prototypes~\cite{boeing777}.
...
Additionally, statistical analyses in clinical trials often rely heavily on p-values, which can misrepresent the reliability of results when not properly contextualized---known as the p-value problem~\cite{goodman1993p}.
